...
export function DecideButtons({
  ...
  // Listen for external decide events (e.g. Enter key from MenuBarApp)
  useEffect(() => {
    function onExternalDecide(e: Event) {
      const detail = (e as CustomEvent).detail;
      if (detail?.sessionId === sessionId && !decided) {
        handleDecide(detail.action);
      }
    }
    window.addEventListener("hexdeck:decide", onExternalDecide);
    return () => window.removeEventListener("hexdeck:decide", onExternalDecide);
  }, [sessionId, decided]);
  ...
  return (
    <div className="flex items-center gap-1.5">
      <button
        ...
        className={`${textSize} font-medium ${px} ${py} rounded bg-dash-green/15 text-dash-green hover:bg-dash-green/25 transition-colors`}
      >
        {approveLabel}
      ...
      <button
        ...
        className={`${textSize} font-medium ${px} ${py} rounded bg-dash-red/15 text-dash-red hover:bg-dash-red/25 transition-colors`}
      >
        Deny
      </button>
    </div>
